...
public class JsonDeserializer {
    ...
    private void processJsonObject(Field field, Object instance, JSONObject jsonObject)
        ...
        if (typeChecker.isTypeHashMap(fieldClass)) {
            ...
            Iterator<String> jsonKeys = (Iterator<String>) jsonObject.keys();
            while (jsonKeys.hasNext()) {
                String jsonKey = jsonKeys.next();

                if (isItemClassNumber) {
                    Number number = (Number) jsonObject.get(jsonKey);
                    hashMap.put(jsonKey, createProcessedNumber(itemClass, number));
                }
                else if (isItemClassStringChar) {
                    String string = jsonObject.getString(jsonKey);
                    hashMap.put(jsonKey, createProcessedText(itemClass, string));
                }
                else if (isItemClassBoolean) {
                    hashMap.put(jsonKey, jsonObject.getBoolean(jsonKey));
                }
                else if (isItemClassJson) {
                    hashMap.put(jsonKey, jsonObject.get(jsonKey));
                }
                else {
                    hashMap.put(jsonKey, deserializeHelper(jsonObject.getJSONObject(jsonKey), itemClass));
                }
            }
